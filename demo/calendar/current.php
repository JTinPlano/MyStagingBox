...
?>
		...
		<div id="demonotice">This is the "Show Events" demo to demonstrate how the live schedule appears to the public.  Any event added is available to the public immediately, and you will be the only one who can edit the listing when it's live, and you must be a registered subscriber to both list and access the listing.  Any event listed here will be upgraded from "Demo" to "Live" once you are registered.  Registration is quick, easy, and free.</div><br>
<?php
$year==date("Y")?$month=date("m"):$month=1;
//echo "line 182 month $month day $day year $year<br>";
$days = array(
	"0" => "Sunday",
	"1" => "Monday",
	"2" => "Tuesday",
...
/*** get distinct city and state to build the state links for each event date	***/
		...
		FROM event
		WHERE Active = '1' and (paid = '1' or demo = '1') and
		(
			(eventtypeid=1 AND dayofmonth = '$thisdate' AND month = '$month')  or
			(eventtypeid=2 AND dayofweek = '$dow')  or
			(eventtypeid=3 AND dayofmonth = '$thisdate')  or
...
END;
					$divid++;
				}  // starts at line	303 - fetching event details
			} // starts at line 293 - getting details on every event for this date
		}  // starts at line 270
...
END;
	$month++;
	echo "\n";
} //starts at line 192 - building each month's calendar for the year
?>
	</div>
<!-- </div> -->